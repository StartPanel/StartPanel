    ...
    */
    ...
    if ($_POST['command'] == "restart") {
        ...
        if ($server->isOnline($_GET['id'])) {
            $ssh->exec('docker exec server'.$_GET['id'].' mc_restart');
        }
        ...
        $ssh->setTimeout(0.5);
        if ($server->isOnline($_GET['id'])) {
            $ssh->exec("docker stop server".$_GET['id']);
        }
        ...
        $ssh->setTimeout(0.5);
        if (!$server->isOnline($_GET['id'])) {
            $cmd_serverID = $_GET['id'];
            $ssh->exec("docker start server".$_GET['id']);
        }
    }
?>